...
class Pad(BaseOp):
    ...
    '''
    slicer_revert_padding_sar: list[int | slice | tuple[Optional[int], Optional[int], Optional[int]]]  # noqa

    def integrate(self: Self, name: str) -> Optional[OpItem]:
        img: NNTensor = self.img
        slicer_revert_padding_sar = self.slicer_revert_padding_sar
        ...
        sanitized_slicer_revert_padding_sar = [
            utils.sanitize_slice(each)
            for each in slicer_revert_padding_sar
        ]
        ...
        return OpItem(
            ...
            op_type=OpType.PAD,
            tensor=img,
            op_params=sanitized_slicer_revert_padding_sar,
        )
